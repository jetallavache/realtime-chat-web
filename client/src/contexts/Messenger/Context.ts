...
import {
  IMessengerContextAction,
  IMessengerContextProps,
  IMessengerContextState,
} from "./interfaces";
...
export const MessengerReducer = (
  state: IMessengerContextState,
  action: IMessengerContextAction,
) => {
  console.log(
    `Messenger context. Message received, - Action: ${action.type}, - Payload: `,
    action.payload,
  );
    ...
    case "remove_user":
      return {
        ...
        users: state.users?.filter(
          (user) => user.uid != (action.payload as string),
        ),
      };
